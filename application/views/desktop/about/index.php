<!--START: Breadcrumbs-->
<aside class="container">
    <ol class="breadcrumbs">
        <li><a href="<?=base_url()?>"><i class="fa fa-home"></i></a></li>
        <li class="active"><span>회사소개</span></li>
    </ol>
</aside>
<!--END: Breadcrumbs-->

<!--START: 메인컨텐츠-->
<article class="container">
    <h2 class="hide">천생연분닷컴 회사소개 네비게이션</h2>
    <img class="wide-banner margin-bottom-30" src="/static/images/about/top_banner.jpg" alt="천생연분닷컴 회사소개 배너">

    <ul class="tab tab-default tab-justified">
        <li class="active"><a href="#" onclick="return false;">천생연분닷컴 본사 회사소개</a></li>
        <li><a href="<?=base_url('about/branch')?>">천생연분닷컴 지사안내</a></li>
    </ul>

</article>

<article id="about" class="container margin-top-50">
    <header>
        <img class="wide-banner" src="/static/images/about/section_01_banner.jpg" alt="허니문 여행사의 기준 천생연분 닷컴, 대한민국 모든 예비 신혼부부에게 설레임과 추억을 선물해 드립니다.">
    </header>
    <img class="wide-banner" src="/static/images/about/section_01_content.jpg">
</article>

<article id="insurance" class="container margin-top-50" id="about-insurance">
    <header>
        <img class="wide-banner" src="/static/images/about/section_02_banner.jpg" alt="천생연분닷컴 보증,보험 가입현황">
    </header>
    <div class="margin-top-50">
        <div class="row">
            <div class="col col-6">

                <div class="togglebar" data-toggle="togglebar">
                    <div class="togglebar-header">
                        <img class="icon" src="/static/images/icons/icon_insurance.gif">&nbsp;BSP 보증보험<i class="caret"></i>
                    </div>
                    <div class="togglebar-body">
                        <img class="img-responsive" src="/static/images/about/insurance_esp.jpg" alt="이행(지급)보증보험 계약변경증권">
                    </div>
                    <i class="plus"></i>
                </div>

                <div class="togglebar margin-top-30" data-toggle="togglebar">
                    <div class="togglebar-header">
                        <img class="icon" src="/static/images/icons/icon_insurance.gif">&nbsp;배상책임 보험<i class="caret"></i>
                    </div>
                    <div class="togglebar-body">
                        <img class="img-responsive" src="/static/images/about/insurance_esp.jpg" alt="배상책임 보험">
                    </div>
                    <i class="plus"></i>
                </div>

            </div>
            <div class="col col-6">

                <div class="togglebar" data-toggle="togglebar">
                    <div class="togglebar-header">
                        <img class="icon" src="/static/images/icons/icon_insurance.gif">&nbsp;기획여행 보증보험<i class="caret"></i>
                    </div>
                    <div class="togglebar-body">
                        <img class="img-responsive" src="/static/images/about/insurance_esp.jpg" alt="기획여행 보증보험">
                    </div>
                    <i class="plus"></i>
                </div>

                <div class="togglebar margin-top-30" data-toggle="togglebar">
                    <div class="togglebar-header">
                        <img class="icon" src="/static/images/icons/icon_insurance.gif">&nbsp;국외여행 보증보험<i class="caret"></i>
                    </div>
                    <div class="togglebar-body">
                        <img class="img-responsive" src="/static/images/about/insurance_esp.jpg" alt="국외여행 보증보험">
                    </div>
                    <i class="plus"></i>
                </div>

            </div>
        </div>
        <img class="wide-banner margin-top-30" src="/static/images/about/total_insurance.jpg" >
    </div>
</article>
<!--END: 메인 컨텐츠-->

<!-- START: HONEYMOON & WEDDING FAIR -->
<article class="container margin-top-50" id="weddingfair">
    <img class="wide-banner" src="/static/images/about/wedding_fair_top.jpg" alt="HONETYMOON & WEDDING FAIR">
    <div id="slide-fair">
        <div class="slide-fair-container">
            <ul>
                <li><img src="/static/images/about/about_fair_big_01.jpg"></li>
                <li><img src="/static/images/about/about_fair_big_02.jpg"></li>
                <li><img src="/static/images/about/about_fair_big_03.jpg"></li>
            </ul>
        </div>
        <div class="slide-fair-list">
            <ul>
                <li class="active"><img src="/static/images/about/about_fair_thumb_01.jpg"></li>
                <li><img src="/static/images/about/about_fair_thumb_02.jpg"></li>
                <li><img src="/static/images/about/about_fair_thumb_03.jpg"></li>
            </ul>
        </div>
    </div>
</article>
<!-- END: HONEYMOON & WEDDING FAIR -->

<!-- START : 본사 찾아오시는 길 -->
<article class="container-fluid margin-top-50" id="location">
    <div class="container">
        <header>
            <img class="wide-banner" src="/static/images/about/section_location_banner.jpg" alt="천생연분닷컴 본사 찾아오시는 길">
        </header>
        <div id="about-location-map">

        </div>
        <div class="location-info">
            <ul>
                <li>T. 02-720-8876</li>
                <li>F. 02-2179-9481</li>
                <li>서울 강남구 봉은사로 437(삼성동 44-10) 소암빌딩 4층</li>
            </ul>
        </div>
        <table class="table transport-info">
            <thead>
            <tr>
                <th><img class="thead-icon" src="/static/images/icons/icon_subway.png" alt="지하철 아이콘">&nbsp;지하철</th>
                <th><img class="thead-icon" src="/static/images/icons/icon_bus.png" alt="버스 아이콘">&nbsp;버스</th>
                <th><img class="thead-icon" src="/static/images/icons/icon_car.png" alt="자동차 아이콘">&nbsp;승용차</th>
            </tr>
            </thead>
            <tbody>
            <tr>
                <td><strong>선정릉역</strong> (9호선/분당선)<br>2번출구 도보 5분거리</td>
                <td>
                    <strong>삼성 2문화센터</strong> (23-212)<br>
                    <strong>라마다 서울 호텔앞</strong> (23-213)<br>
                    <span class="text-color-blue">간선</span>&nbsp;351 |
                    <span class="text-color-green">지선</span>&nbsp;3412 |
                    <span class="text-color-green">마을</span>&nbsp;강남07
                </td>
                <td>
                    <strong>서울시 강남구 봉은사로 437<br>
                    (소암빌딩4F)</strong> ※현대자동차 건물
                </td>
            </tr>
            </tbody>
        </table>
    </div>
</article>
<!-- END : 본사 찾아오시는 길-->

<!-- START:계좌 안내-->
<article class="container-fluid" id="account">
    <h2 class="hide">천생연분닷컴 계좌번호 안내</h2>
    <div class="container">
        <table class="table table-account margin-top-30" summary="천생연분닷컴 계좌번호안내">
            <colgroup>
                <col width="260" />
                <col width="*" />
                <col width="260" />
            </colgroup>
            <thead>
            <tr>
                <th>은행명</th>
                <th>계좌번호</th>
                <th>입금자명</th>
            </tr>
            </thead>
            <tbody>
            <tr>
                <td class="text-center"><img src="/static/images/icons/icon_kb.png"></td>
                <td class="account-number">268801-04-202744</td>
                <td class="text-center">(주)네이버네트워크</td>
            </tr>
            <tr>
                <td class="text-center"><img src="/static/images/icons/icon_kb.png"></td>
                <td class="account-number">431802-01-373491</td>
                <td class="text-center">(주)네이버네트워크</td>
            </tr>
            </tbody>
        </table>
    </div>
</article>
<!-- END:계좌 안내-->

<script type="text/javascript" src="https://openapi.map.naver.com/openapi/v3/maps.js?clientId=qgDDeNxLd_swk2lKKlxf"></script>
<script>
$(function(){
    $("#slide-fair > .slide-fair-list > ul >li").on('click', function(){
        var index = $(this).index();
        var slide_height = 400;
        $("#slide-fair > .slide-fair-list > ul >li").removeClass("active");
        $("#slide-fair > .slide-fair-list > ul >li").eq(index).addClass("active");

        $("#slide-fair > .slide-fair-container > ul").stop().animate({'top': -1 * (index * slide_height) + "px"}, 300);
    });
});
var mapX = 37.511818;
var mapY = 127.047605;
var position = new naver.maps.LatLng(mapX, mapY);
var mapOptions = { center: position, zoom: 12 };
var map = new naver.maps.Map('about-location-map', mapOptions);
var marker = new naver.maps.Marker({ position: position, map: map});
var contentString = ['<div id="map-info-window">','<h4>천생연분닷컴</h4>','<p>서울 강남구 봉은사로 437(삼성동 44-10) 소암빌딩 4층</p>','</div>'].join('');
var infoWindow = new naver.maps.InfoWindow({ content : contentString});
infoWindow.open(map, marker);
</script>
